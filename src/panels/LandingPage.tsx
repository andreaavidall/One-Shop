...
import { Search, Sparkles, Building2, ShieldCheck, Zap, ArrowRight, Star, HelpCircle, Layers, CheckCircle2 } from 'lucide-react';
...
export const LandingPage: React.FC<LandingPageProps> = ({ onStartShopping }) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#09090b] text-zinc-900 dark:text-zinc-50 transition-colors duration-300">
      {/* Top Navbar */}
      <header className="sticky top-0 z-40 w-full border-b border-zinc-200/80 dark:border-zinc-800/80 bg-white/80 dark:bg-[#09090b]/80 backdrop-blur-md">
        <div className="max-w-[1400px] mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-bold tracking-widest bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 px-1.5 py-0.5 rounded ml-1 border border-zinc-200 dark:border-zinc-700">
              B2B
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => onStartShopping()}
              className="bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-50 dark:hover:bg-zinc-200 text-white dark:text-zinc-950 font-semibold px-4 py-2 rounded-lg text-sm transition-colors shadow-sm cursor-pointer"
            >
              Acceso Clínicas
            </button>
          </div>
      ...
      <section className="relative pt-20 pb-24 overflow-hidden">
        ...
        <div className="absolute top-1/3 left-1/3 w-[300px] h-[300px] bg-purple-500/10 dark:bg-purple-500/5 rounded-full blur-[100px] pointer-events-none" />

        <div className="max-w-[1400px] mx-auto px-6 text-center relative z-10">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 text-xs font-semibold mb-6 border border-blue-200/50 dark:border-blue-900/30">
            <Sparkles size={12} />
            La primera plataforma agregadora inteligente de odontología
          </div>
          
          <h1 className="text-4xl md:text-6xl font-black tracking-tight max-w-4xl mx-auto leading-[1.1] mb-6 text-zinc-950 dark:text-white">
          ...
          <p className="text-base md:text-xl text-zinc-500 dark:text-zinc-400 max-w-2xl mx-auto leading-relaxed mb-10">
            Encuentra productos de múltiples proveedores, compara opciones en tiempo real y compra de forma inteligente desde una única plataforma unificada.
          </p>

          {/* Smart Search Bar Protagonist */}
          <form onSubmit={handleSearchSubmit} className="max-w-2xl mx-auto mb-8">
            <div className="relative p-2 rounded-2xl bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/20 dark:shadow-none flex items-center gap-2 focus-within:ring-2 focus-within:ring-blue-500/50 transition-all">
              <div className="flex items-center gap-2 pl-3 flex-1">
                ...
                <input
                  ...
                  placeholder='Busca "Resina Filtek", "Guantes" o prueba con "Necesito todo para una endodoncia"...'
                  className="bg-transparent border-none text-sm w-full focus:outline-none text-zinc-900 dark:text-zinc-100 placeholder-zinc-400"
                />
              </div>
              <button
                type="submit"
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors flex items-center gap-1.5 shadow-md shadow-blue-500/10 cursor-pointer"
              >
                Buscar
                <ArrowRight size={14} />
              </button>
            </div>
            <div className="flex flex-wrap justify-center gap-2 mt-4 text-xs text-zinc-400 dark:text-zinc-500">
              <span className="font-semibold text-zinc-500 dark:text-zinc-400">Prueba con:</span>
              {['Resina Filtek', 'Guantes', 'Necesito todo para una endodoncia', 'Tengo S/15,000 para equipar'].map((tag, idx) => (
                <button
                  key={idx}
                  type="button"
                  onClick={() => setSearchQuery(tag)}
                  className="hover:text-blue-500 dark:hover:text-blue-400 transition-colors underline cursor-pointer"
                >
                  {tag}
                </button>
              ))}
          ...
          <div className="flex justify-center gap-4">
            <button
              onClick={() => onStartShopping()}
              className="bg-zinc-950 hover:bg-zinc-800 dark:bg-white dark:hover:bg-zinc-200 text-white dark:text-zinc-950 font-bold px-6 py-3 rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer"
            >
              Comenzar a comprar
              <ArrowRight size={16} />
            </button>
            <button
              onClick={() => onStartShopping()}
              className="bg-white hover:bg-zinc-100 dark:bg-zinc-900 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-800 font-semibold px-6 py-3 rounded-xl shadow-sm transition-all cursor-pointer"
            >
              Explorar productos
            </button>
          </div>
      ...
      <section className="py-20 border-t border-zinc-200 dark:border-zinc-900 bg-white dark:bg-[#0c0c0f]/30">
        <div className="max-w-[1400px] mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold tracking-tight mb-4 text-zinc-950 dark:text-white">Una experiencia de compra inteligente</h2>
      ...
      <section className="py-20 border-t border-zinc-200 dark:border-zinc-900">
        <div className="max-w-[1400px] mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-zinc-500 dark:text-zinc-400 max-w-xl mx-auto text-sm">
              Conectamos el stock de los proveedores más importantes del país para que tú solo tengas que hacer un clic.
            </p>
          </div>
